...
void statement(symset fsys)
{
	...
	{ //chunk, compound_statement
		...
		{//sysÔÚ²»ÔÚstatementµÄÇ°×ºÖÐ,while true, there are other more statements before 'end'
...
<<<<<<< HEAD
		...
		int cxiaddr = 0; //caseåŽé¢æ•°å­—(å¸¸é‡ä¸è¡Œ)
		int cxcase = 0;//æ‰§è¡Œè¯­å¥çš„åœ°å€
		int cxend[50];  //æ¯ä¸ªcaseçš„æ¡ä»¶åˆ¤æ–­çš„åœ°å€
=======
		//switch (a) {caseÊý×Ö:Óï¾ä;}
		int cxiaddr = 0; //caseºóÃæÊý×Ö(Ã»°ì·¨Ð´³£Á¿ÁË)
		int cxcase = 0;//Ö´ÐÐÓï¾äµÄµØÖ·
		int cxend[50];  //Ã¿¸öcaseµÄÌõ¼þÅÐ¶ÏµÄµØÖ·
...
>>>>>>> f58ea94f145f49885600227ea8cb85065cdda637
			...
			if (sym != SYM_IDENTIFIER)
				...
				else if (table[i].kind == ID_VARIABLE)//¡¾´ý»áÒª gen(LOD
				{
					mk = (mask*)&table[i];
					cxiaddr = mk->address;
				}
